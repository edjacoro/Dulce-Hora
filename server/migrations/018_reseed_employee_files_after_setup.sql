...
do update set
  ...
  active = true,
  source = case
    when employees.source is null or employees.source = 'manual' or employees.source like 'default-schedule%' then excluded.source
    else employees.source
  end,
  schedule_template = case
    when coalesce(nullif(employees.schedule_template, ''), '{}') = '{}' or employees.source like 'default-schedule%' then excluded.schedule_template
